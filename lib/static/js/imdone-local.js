define([
  ...
  imdone.md = function(md) {
    ...
    html = html.replace(/(<a.*?href=")(.*?)(".*?)>/ig, function(anchor, head, href, tail) {
      if (externalLinks.test(href)) {
        ...
        return head + href + tail + ' title="Filter by ' + filterBy + '">';   
      } else {
  ...
  //[Tak a look at this <https://speakerdeck.com/ammeep/unsuck-your-backbone>, <http://amy.palamounta.in/2013/04/12/unsuck-your-backbone/>](#doing:0)
  //--------------------------------------Backbone Models---------------------------------------
  ...
  imdone.filter = function(filter) {
    ...
    if (filter) imdone.filterField.val(filter);
    filter = imdone.filterField.val();
  ...
  //[Implement delete file functionality](#done:30)
  ...
  imdone.init = function() {
      ...
      $(window).bind('keydown', 'I', function(e){
        ...
        if (imdone.previewMode && imdone.editMode) {
            imdone.app.navigate("project" + imdone.cwd(), {trigger:true});
            e.preventDefault();
            e.stopPropagation();
            return false;
  ...
  var AppRouter = Backbone.Router.extend({
      routes: {
          ...
          "filter/*filter" : "filterRoute", //[Filter route so links can change filter](#done:0)
