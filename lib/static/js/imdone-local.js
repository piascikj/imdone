define([
  ...
  imdone.md = function(md) {
    ...
    //[Let mailto links open email client](#doing:30)
  ...
  //[Take a look at this <https://speakerdeck.com/ammeep/unsuck-your-backbone>, <http://amy.palamounta.in/2013/04/12/unsuck-your-backbone/>](#doing:50)
  //--------------------------------------Backbone Models---------------------------------------
  ...
  imdone.search = function(params) {
    var project = params && params.project || imdone.cwd();
    if (project) {
      $.get("/api/search" + project + "?query=" + params.query, function(data){
  ...
  //[Implement delete file functionality](#done:40)
  ...
  imdone.init = function() {
      ...
      imdone.searchForm.submit(function(event) {
        ...
        var dest = "search?project=" + imdone.cwd() + "&query=" + imdone.searchField.val();
      ...
      imdone.searchResultsBtn.click(function() {
        if (imdone.searchResults.is(":visible")) {
  ...
  var AppRouter = Backbone.Router.extend({
      routes: {
          ...
          "filter/*filter" : "filterRoute", //[Filter route so links can change filter](#done:10)
      ...
      searchRoute: function(qs) {
        var params = imdone.parseQueryString(qs);
        console.log("search route: " + JSON.stringify(params,null,3));
        if (!imdone.cwp()) {
          imdone.getKanban({project:params.project, noPaint:true, callback:function() {
