define([
  ...
  'pnotify',
  'hotkeys'
], function(_, $, Backbone, Handlebars, JSON, io, marked, store, Search) {
  ace = window.ace;
  ...
  imdone.md = function(md) {
    ...
    //[Let mailto links open email client](#done:0)
  ...
  //[Take a look at this <https://speakerdeck.com/ammeep/unsuck-your-backbone>, <http://amy.palamounta.in/2013/04/12/unsuck-your-backbone/>](#doing:80)
  //--------------------------------------Backbone Models---------------------------------------
  ...
  imdone.search = function(params) {
    ...
    if (project) {
      ...
      search.fetch({success: function(model, response)  {
          // [Put search in a view](#doing:0)
          var template = Handlebars.compile($("#search-results-template").html());
          var last = model.get("total")+model.get("offset");
          var context = {project:project,results:response,last:last};
  ...
  imdone.hideSearchResults = function() {
  ...
  //[Implement delete file functionality](#done:60)
  ...
  imdone.init = function() {
      ...
      //[Use Vim keyboard bindings](#planning:90)
      ...
      //close the source
      imdone.closeFileBtn.live('click', function() {
          imdone.app.navigate("project" + imdone.cwd(), {trigger:true});
      });
      ...
      imdone.searchForm.submit(function(event) {
        ...
        var dest = "search?project=" + imdone.cwd() + 
                   ...
                   "&offset=0&limit=100";
        console.log("Calling navigate with dest:" + dest);
  ...
  var AppRouter = Backbone.Router.extend({
      routes: {
          ...
          "filter/*filter" : "filterRoute", //[Filter route so links can change filter](#done:30)
      ...
      searchRoute: function(qs) {
        var params = imdone.parseQueryString(qs);
        if (!imdone.cwp()) {
          imdone.getKanban({project:params.project, callback:function() {
            imdone.paintProjectsMenu();
