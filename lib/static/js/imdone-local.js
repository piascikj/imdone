...
imdone.initUpdate = function() {
  if ("WebSocket" in window)
  {
     ...
     { 
        ...
        if (window.imdone && window.imdone.data && window.imdone.data.lastUpdate && (new Date(obj.lastUpdate) > new Date(window.imdone.data.lastUpdate))) {
          if (imdone.server == undefined ) {
            imdone.server = {lastUpdate: obj.lastUpdate};
          } else {
            imdone.server = obj
          };
