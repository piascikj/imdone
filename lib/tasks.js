...
module.exports = {
	...
	getTasks: function(data, taskProcessor) {
		...
		data.replace(taskregex, function(md, text, list, order, pos) {
			if (!self.isCode(data, pos)) {
				//[add the line number of the task by finding position and counting newlines prior - 0.1.4](#archive:380)
				//[Use line number when loading page in github - 0.1.4](#archive:360)
				var line = (data.substring(0,pos).match(/\n/g)||[]).length + 1;
				//[For task modification, store text as text and create another property for html](#todo:0)
				var task = {
					md:md,
					text:marked(text),
					list:list,
	...
	modifyTask: function(file, task) {
		...
		var self = this,
			n = 0;
		var content = file.content.replace(taskregex, function(md, text, list, order, pos) {
			if (self.isCode(md, pos)) return md;
