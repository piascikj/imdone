...
var utils = module.exports = {
	...
	getTasks: function(data, file, taskProcessor) {
		...
		data.replace(taskregex, function(md, text, list, order, pos) {
			if (utils.isValidTask(data, file, pos)) {
				...
				//[For task modification, store text as text and create another property for html](#done:50)
				var task = {
					md:md,
					text:text,
	...
	isValidTask: function(data, file, pos) {
		...
		if (lang && lang == "markdown") {
			...
			return this.isComment(data, lang.symbol, pos);
		}

		return false;
	},

	//[Test isValidTask and isComment](#doing:0)
	isComment: function(data, symbol, pos) {
		var done = false, beforeTask = "", comment = false;
		for(var i=pos; done == false; i--) {
			beforeTask = data.substring(i,pos);
			if (/\n/.test(beforeTask)) {
			done = true
			} else if (beforeTask.indexOf(symbol > -1)) {
