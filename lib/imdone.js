 ...
 */
//[Implement hide functionality to hide a list from board](#archive:240)
// Nodejs libs.
var fs = require('fs');
...
//[Fix task regex so that tasks that appear in markdown as code aren't displayed - 0.1.6](#archive:330)
...
//[When moving task to top of list, it goes to second place.  Need to fix this](#archive:200)
...
imdone.Project.prototype.processFiles = function(files, callback) {
	try {
		...
		_.each(files, function(file, i) {
			...
			//[Make this an async file read](#archive:290)
			var data = fs.readFile(fullPathFile, 'utf8', function (err, data) {
				...
				data.replace(imdone.taskregx, function(md, text, list, order, pos) {
					if (!self.isCode(data, pos)) {
						...
						//[Use line number when loading page in github - 0.1.4](#archive:350)
...
imdone.Project.prototype.update = function(files) {
	...
	_.each(files, function(file) {
		if (!self.isPaused(file)) {
			//[Store last updated time, and check to see if we should process - 0.1.3](#archive:370)
...
imdone.Project.prototype.watchFiles = function(path) {
	...
	watchr.watch({
    	...
    	//[Use ignoreCustomPatterns](#archive:280)
...
/*
	...
	- [Modify process.env to contain the port info for cliService](#archive:210)
