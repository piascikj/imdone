...
var git = require('./git').init(imdone);

//[Fix task regex so that tasks that appear in markdown as code aren't displayed](#doing:0)
var taskregx = /\[(.+?)\]\(#([\w-]+?):(\d+?\.{0,1}\d*?)\)/g;
 
imdone.getURL = function(file, line) {
	if (imdone.config.github) {
		return git.getURL(file,line);
	} else {
		return file;
	}
};
	...
	try {
		...
		_.each(files, function(file, i) {
			if (imdone.config.include.test(file) == true && imdone.config.exclude.test(file) == false && !fs.statSync(file).isDirectory()) {
				...
				data.replace(taskregx, function(md, text, list, order, pos) {
					if (imdone.tasks[file] == undefined) imdone.tasks[file] = {lastUpdate:lastUpdate,tasks:{}};
					imdone.lastUpdate = lastUpdate;
					//[add the line number of the task by finding position and counting newlines prior - 0.1.4](#done:20)
					//[Use line number when loading page in github - 0.1.4](#done:10)
					var line = (data.substring(0,pos).match(/\n/g)||[]).length;
					if (line > 0) line++;
					var task = {
						path:file,
						url: imdone.getURL(file, line),
						md:md,
						text:marked(text),
						list:list,
...
imdone.update = function(files) {
	_.each(files, function(file) {
		if (!imdone.isPaused(file)) {
			//[Store last updated time, and check to see if we should process - 0.1.3](#done:30)
...
imdone.initConfig = function() {
	...
	console.log("config:" + JSON.stringify(imdone.config, null, 3));
