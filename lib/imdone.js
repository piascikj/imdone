...
imdone.Project.prototype.getFiles = function(path) {
	...
	_.each(files, function(file) {
		...
		if (fs.statSync(file).isDirectory()) {
			if (!self.config.exclude.test(self.relativePath(file) + "/")) {
				out.dirs.push(_.extend({name:name,path:file},self.getFiles(file)));
			}

		} else if (fs.statSync(file).isFile()) {
			out.files.push({name:name,path:file});
		}
	});
	return out;
};
...
imdone.Project.prototype.watchFiles = function(path) {
	//[Test watchr on hundreds of directories](#todo:60)
	var self = this;
	watchr.watch({
    	path: path,
...
/*
	- [Modify $HOME/.imdone/config.js to edit the project directories watched by imdone](#todo:110)
