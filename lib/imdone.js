 ...
 */
//[Implement hide functionality to hide a list from board](#done:0)
// Nodejs libs.
var fs = require('fs');
...
imdone.Project = function(path) {
	...
	this.initConfig( function() {
		self.loadListData(function() {
			self.cwd = self.path; //[Stop using process.cwd, use the path instead](#archive:80)
...
//[Fix task regex so that tasks that appear in markdown as code aren't displayed - 0.1.6](#archive:100)
...
imdone.Project.prototype.processFiles = function(files, callback) {
	try {
		...
		_.each(files, function(file, i) {
			...
			//[Make this an async file read](#archive:50)
			var data = fs.readFile(fullPathFile, 'utf8', function (err, data) {
				...
				data.replace(imdone.Project.taskregx, function(md, text, list, order, pos) {
					if (!imdone.Project.isCode(data, pos)) {
						...
						//[add the line number of the task by finding position and counting newlines prior - 0.1.4](#archive:130)
						//[Use line number when loading page in github - 0.1.4](#archive:120)
...
imdone.Project.prototype.update = function(files) {
	...
	_.each(files, function(file) {
		if (!self.isPaused(file)) {
			//[Store last updated time, and check to see if we should process - 0.1.3](#archive:140)
...
imdone.Project.prototype.watchFiles = function(path) {
	...
	watchr.watch({
    	...
    	//[Use ignoreCustomPatterns](#archive:40)
