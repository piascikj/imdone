...
imdone.Project = function(path) {
	...
	this.initConfig();
	this.loadListData();
	this.cwd = process.cwd(); //[Stop using process.cwd](#doing:10)
...
imdone.Project.prototype.modifyTask = function(task, changes) {
	...
	var n = 0;
	data = data.replace(imdone.taskregx, function(md, text, list, order) {
...
imdone.Project.prototype.initConfig = function() {
	...
	if (fs.existsSync(this.configFile)) {
		...
		var inStr = fs.createReadStream(require.resolve(this.defaultConfig));
