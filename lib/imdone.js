...
//[Fix task regex so that tasks that appear in markdown as code aren't displayed - 0.1.6](#archive:340)
			...
			var data = fs.readFile(fullPathFile, 'utf8', function (err, data) {
				...
				data.replace(imdone.taskregx, function(md, text, list, order, pos) {
					if (!self.isCode(data, pos)) {
						...
						//[add the line number of the task by finding position and counting newlines prior - 0.1.4](#archive:370)
						//[Use line number when loading page in github - 0.1.4](#archive:360)
...
imdone.Project.prototype.update = function(files) {
	...
	_.each(files, function(file) {
		if (!self.isPaused(file)) {
			//[Store last updated time, and check to see if we should process - 0.1.3](#archive:380)
