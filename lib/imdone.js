		...
		_.each(files, function(file, i) {
			if (imdone.config.include.test(file) == true && imdone.config.exclude.test(file) == false && !fs.statSync(file).isDirectory()) {
				...
				data.replace(taskregx, function(md, text, list, order, pos) {
					...
					//[add the line number of the task by finding position and counting newlines prior - 0.1.4](#done:10)
					//[Use line number when loading page in github - 0.1.4](#done:0)
...
imdone.update = function(files) {
	_.each(files, function(file) {
		if (!imdone.isPaused(file)) {
			//[Store last updated time, and check to see if we should process - 0.1.3](#done:20)
