 ...
 */
//[Implement hide functionality to hide a list from board](#archive:190)
// Nodejs libs.
var fs = require('fs');
...
//[Fix task regex so that tasks that appear in markdown as code aren't displayed - 0.1.6](#archive:280)
imdone.taskregx = /\[(.+?)\]\(#([\w\-]+?):(\d+?\.{0,1}\d*?)\)/g;
...
//[When moving task to top of list, it goes to second place.  Need to fix this](#archive:150)
...
imdone.Project.prototype.modifyTask = function(data,task) {
	var self = this,
		n = 0;
	data = data.replace(imdone.taskregx, function(md, text, list, order, pos) {
...
imdone.Project.prototype.filesToProcess = function(files, showDirs) {
	...
	_.each(files, function(file, i) {
		...
		if (self.config.include.test(relPathFile) == true && self.config.exclude.test(relPathFile) == false && (fs.statSync(file).isFile() || showDirs)) {
			passed.push(file);
		}
	});
...
imdone.Project.prototype.processFiles = function(files, callback) {
	try {
		...
		_.each(files, function(file, i) {
			...
			//[Make this an async file read](#archive:240)
			var data = fs.readFile(fullPathFile, 'utf8', function (err, data) {
				...
				data.replace(imdone.taskregx, function(md, text, list, order, pos) {
					if (!self.isCode(data, pos)) {
						self.tasks[relPathFile] = self.tasks[relPathFile] || {lastUpdate:lastUpdate,tasks:{}};
						self.lastUpdate = lastUpdate;
						//[add the line number of the task by finding position and counting newlines prior - 0.1.4](#archive:310)
...
imdone.Project.prototype.update = function(files) {
	...
	_.each(files, function(file) {
		if (!self.isPaused(file)) {
			//[Store last updated time, and check to see if we should process - 0.1.3](#archive:320)
...
imdone.Project.prototype.watchFiles = function(path) {
	...
	watchr.watch({
    	...
    	//[Use ignoreCustomPatterns](#archive:230)
...
/*
	...
	- [Modify process.env to contain the port info for cliService](#archive:160)
