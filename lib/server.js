 ...
 */
    ...
    //[Move getSource to imdone.js](#doing:30)
    ...
    //[Move saveSource to imdone.js](#doing:40)    
	...
	function removeSource(req, res) {
		...
		if (project.path && /^\.\./.test(path) == false) {
			fs.unlink(filePath, function(err, data) {
				if (err) {
					res.send({error:"Unable to delete source"});
					return;
				}			
	...
	server.start = function(imdone, callback) {
		...
		//[API should be more RESTful](#planning:140)
		/*
			/api/tasks
			/api/lists
			/api/projects
