define([
  ...
  // DOING:0 Show a modal on startup that advertises chrome app and gives a poll id:4430
  // DONE:30 Use [spin.js](http://fgnass.github.io/spin.js/#?lines=15&length=24&width=9&radius=60&corners=0.1&rotate=0&trail=60&speed=0.5&direction=1&hwaccel=on) for loading gif id:4370
  //pnotify options
  ...
  // #TODO:60 Replace format with _.template  id:4375
  ...
  imdone.md = function(md) {
    ...
    var html = marked(md);
    // #TODO:40 everything above this should be in imdone-core Repository or File id:4376
    ...
    // ARCHIVE:830 For markdown files, find tasks links and give them a badge id:4413
    // ARCHIVE:360 For internal inks, take them to the page id:4432
  ...
  //#TODO:70 Take a look at this <https://speakerdeck.com/ammeep/unsuck-your-backbone>, <http://amy.palamounta.in/2013/04/12/unsuck-your-backbone/> id:4374
  ...
  // PLANNING:90 add notify and undo for move id:4378
  ...
  imdone.search = function(params) {
    ...
    if (project) {
      ...
      search.fetch({success: function(model, response)  {
          // #TODO:90 Put search in a view.  [What is a view? - Backbone.js Tutorials](http://backbonetutorials.com/what-is-a-view/) id:4379
  ...
  imdone.filter = function(filter) {
    ...
    if (filter) {
      ...
      // ARCHIVE:770 Use a regex for filter and create button to filter by files of selected tasks id:4415
  ...
  imdone.paintKanban = function(data) {
    if (!data.busy && !imdone.editMode) {
      ...
      if (data.readme) {
        // ARCHIVE:160 Fix readme href id:4434
    ...
    if (data.mods.length > 0) {
      ...
      if (fileUpdate && imdone.source && fileUpdate.file === imdone.source.path) {
        client.getFile(imdone.currentProjectId(), imdone.source.path, function(data) {
          ...
          // #TODO:20 How do we check for deleted??? id:4377
        });
      }
    }
  };
  ...
  imdone.addFileToHistory = function() {
    ...
    //ARCHIVE:900 Don't pop, shift id:4381
  ...
  imdone.getSource = function(params) {
    ...
    //ARCHIVE:880 We have to convert the source api url URL first id:4384
    ...
    client.getFile(params.project, params.path, params.line, function(data){
        ...
        //ARCHIVE:750 Update file-path on edit button id:4417
  ...
  // ARCHIVE:130 Fix markdown language mode for editor id:4436
  //Show the editor
  imdone.showEditor = function(e) {
    ...
    // ARCHIVE:790 User should be able to set global ace confiuration and have it saved to config.js id:4380
  ...
  imdone.showPreview = function(e) {
    ...
    if (imdone.isMD()) {
      ...
      // ARCHIVE:100 Fix scrollSpy id:4383
  ...
  // ARCHIVE:90 Fix toc click id:4386
  ...
  //ARCHIVE:950 User should be notified when a file has been modified id:4419
  ...
  imdone.removeSource = function() {
    client.removeFile(imdone.currentProjectId(), imdone.source.path,
      ...
      function(data) {
        // PLANNING:30 Make this pnotify default for all errors! id:4437
        imdone.fileNotify = $.pnotify({
  ...
  //ARCHIVE:890 Implement delete file functionality id:4382
  ...
  imdone.initEditor = function() {
    ...
    //ARCHIVE:800 Use Vim keyboard bindings id:4388
    ...
    // ARCHIVE:20 This should ask for a list and order id:4389
    imdone.editor.commands.addCommand({
      name: 'makeTask',
  ...
  // ARCHIVE:820 Clean up init before implementing backbone views id:4421
  imdone.init = function() {
    ...
    $(document).on('click','.source-link', function(e) {
      ...
      //ARCHIVE:380 Show the current task as notification with <http://pinesframework.org/pnotify/> id:4438
      $.pnotify({
        title: list,
    ...
    // PLANNING:110 Use [egdelwonk/SlidePanel](https://github.com/egdelwonk/slidepanel) for opening files and removing clutter id:4385
    function openFile() {
      // ARCHIVE:180 Create a new file based on path and project with call to PUT /api/source.  If get fails call saveSource first to create the file id:4391
    ...
    //ARCHIVE:390 Apply filter when kanban is reloaded id:4392
    ...
    // Listen for hide
    // PLANNING:120 Show prompt if list is large before showing id:4423
  ...
  var AppRouter = Backbone.Router.extend({
      ...
      initialize: function() {
        ...
        //ARCHIVE:400 Construct views and models in here! id:4439
